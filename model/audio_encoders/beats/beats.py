...
class TransformerEncoder(nn.Module):
    def __init__(self, args, checkpointing=False):
        super().__init__()
        ...
        for i, layer in enumerate(self.layers):
            ...
            if not self.training or (dropout_probability > self.layerdrop):
                ...
                if self.checkpointing:
                    x, z, pos_bias = torch.utils.checkpoint.checkpoint(layer, x, padding_mask, False, pos_bias)
                else:
                    x, z, pos_bias = layer(x, padding_mask, False, pos_bias)
...
class BEATs(nn.Module):
    def __init__(
        ...
        self.encoder = TransformerEncoder(cfg, checkpointing)
    ...
    def forward(self, fbank):
        ...
        ### b, 512 , 512
        features = features.transpose(1, 2) ##b,512,512
